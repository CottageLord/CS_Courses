...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    triangle_top = std::min(triangle_top, max_height);
    triangle_btm = std::max(triangle_btm, 0);
    triangle_rht = std::min(triangle_rht, max_height);
    triangle_lft = std::max(triangle_lft, 0);

    // define the super sample scale - how many pieces do we divide each pixel
    ...
    */
    ...
    for (int y {triangle_btm}; y < triangle_top; ++y)
    {
        for (int x {triangle_lft}; x < triangle_rht; ++x)
        {
            index = y * max_width + x;
