/******************************************
 * Class        : CSC-452 Phase 3 a
 * File         : phase3a.c 
 * Name         : Yang Hu / Shu Lin
 * Description  : Static Identity Page Table basic functionalities (create, free, etc.)
 ******************************************/
...
PageTableFree(PID pid)
{
    ...
    int result = P1_SUCCESS;
    USLOSS_PTE *table = NULL;
    //Does nothing if P3_VmInit has not been called.
    if (initialized) {
        result = P3PageTableGet(pid, &table); //--- crush here, don't know why
        if(result == P1_SUCCESS) {
            free(table);
            // pageTables[pid] = NULL;
            P3PageTableSet(pid, NULL);
        }
    }
    return result;
}
